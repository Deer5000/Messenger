  ...
  if (!options) {
    [NSException raise:kFirebaseCoreErrorDomain
                format:@"`[FIRApp configure];` (`FirebaseApp.configure()` in Swift) could not find "
                       @"a valid GoogleService-Info-4.plist in your project. Please download one "
                       @"from %@.",
                       kPlistURL];
  }
  ...
  if ([app configureCore]) {
    ...
    [NSException raise:kFirebaseCoreErrorDomain
                format:@"Configuration fails. It may be caused by an invalid GOOGLE_APP_ID in "
                       @"GoogleService-Info-4.plist or set in the customized options."];
  }
}
...
+ (NSError *)errorForMissingOptions {
  NSDictionary *errorDict = @{
    NSLocalizedDescriptionKey :
        @"Unable to parse GoogleService-Info-4.plist in order to configure services.",
    NSLocalizedRecoverySuggestionErrorKey :
        @"Check formatting and location of GoogleService-Info-4.plist."
  };
...
+ (NSError *)errorForInvalidAppID {
  NSDictionary *errorDict = @{
    ...
    NSLocalizedRecoverySuggestionErrorKey :
        @"Check formatting and location of GoogleService-Info-4.plist or GoogleAppID set in the "
        @"customized options."
  };
